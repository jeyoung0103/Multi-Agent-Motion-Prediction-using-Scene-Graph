...
class AttentionLayer(MessagePassing):
    ...
    def __init__(self,
        ...
        # Analysis helper: when True, the layer stores the latest attention map
    ...
    def message(self,
        ...
        if self.record_attn:
            # Shape: attn [E, H], edge_index [2, E]
            self._last_attn = {
                'attn': attn.detach().cpu(),
                'edge_index': getattr(self, '_last_edge_index', None),
                'num_src': self._last_sizes[0] if hasattr(self, '_last_sizes') else None,
                'num_dst': self._last_sizes[1] if hasattr(self, '_last_sizes') else None,
            }
    ...
    def _attn_block(self,
        ...
        if self.record_attn:
            # Persist metadata so downstream analysis can recover dense heatmaps
            self._last_edge_index = edge_index.detach().cpu()
            self._last_sizes = (x_src.size(0), x_dst.size(0))
